...
class Game():
    ...
    def end(self, text):
        ...
        if int(self.data[0]) < self.score:
            self.save_to_data_file(self.score, self.data[1], self.data[2], self.data[3], self.data[4])
        self.read_data_file()
        self.add_coins()
        self.__init__()
        self.menu()
    ...
    def check_keys(self):
        ...
        if keys[pygame.K_SPACE] and not self.click:
            ...
            if len(self.projectiles) <= 2 and self.rpg_chosen == True:
                self.click = True
    ...
    def menu(self):
        ...
        while True:
            ...
            self.kurwa = self.font.render("Highest Score: " + str(self.data[0]) + " Coins: " + str(self.data[1]) , True, (0, 0, 0))
